...
## AC 검증 (`@ac-verifier`, Green 이후)

| AC  | 판정    | 근거                                                                                             |
| --- | ------- | ------------------------------------------------------------------------------------------------ |
| 1   | ✅ PASS | 서비스 테스트가 일곱 칸을 `toEqual`로 못 박고, 경고 수의 180일 창은 통합 테스트가 실 DB로 본다   |
| 2   | ✅ PASS | 이름·이메일 부분 일치를 진짜 Postgres에 심은 6명으로 검증                                        |
| 3   | ✅ PASS | 시/도 단독·시/군/구 단독·둘 다·검색어와 AND, 그리고 **주소 없는 회원이 빠지는 것**까지           |
| 4   | ✅ PASS | 비활성화 회원이 목록에 남고 `DEACTIVATED`가 되며, 만료·해제된 제재는 `SUSPENDED`가 아니다        |
| 5   | ✅ PASS | 목록 행이 상세로 링크되고, 다섯 덩이를 통합 테스트가 각각 실 DB로 확인                           |
| 6   | ✅ PASS | 컨트롤러 테스트가 **진짜 Nest 앱을 HTTP로 띄우고 진짜 `AdminGuard`를 쓴다.** 403과 200을 모두 봄 |

**AC 6개 전부 PASS. 가짜 테스트 없음. 미충족 0건.**

> 검증자가 "`memberStatusOf` 순수 함수 테스트 파일을 못 찾았다"고 적었으나 이는
> 탐색 누락이다. `packages/shared/src/admin.test.ts`에 세 건이 실제로 있고 통과한다.
...
| 분류              | 건수 | 내용                                                                                                               |
| ----------------- | ---- | ------------------------------------------------------------------------------------------------------------------ |
| 🔴 즉시 수정 필요 | 0    | 타입 오류 0, 비밀값 0, `NEXT_PUBLIC_` 오용 0, `console.*` 0, **의존성 변경 0**                                     |
| 🟡 권장 수정      | 1    | 상세의 다섯 목록에 **상한이 없다.** 원장·리뷰가 많은 회원의 응답이 커진다 — 페이징은 이 이슈 범위 밖이라 다음 이슈 |
| ⚪ 무시 가능      | 17   | `pnpm audit` 결과. **이 브랜치가 의존성을 하나도 바꾸지 않았고** 전부 `security-exceptions.md`에 판정된 기존 항목  |

이 이슈가 다루는 개인정보(이메일·주소·원장)는 전부 `AdminGuard` 뒤에 있다.
**계좌 정보는 응답에 넣지 않았다** — §11.3에 있지만 AC5의 다섯 덩이가 아니고,
평문 계좌 열람 경로를 하나 더 만들면 감사 로그가 두 곳으로 갈린다 (#34가 이미 한다).
