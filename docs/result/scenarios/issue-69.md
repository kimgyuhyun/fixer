...
`/security-review 69` (2026-09-19). 타입 오류 0, 전체 테스트 통과 상태에서 수행했다.

### 🔴 즉시 수정 필요

| 항목                                              | 판단                                                                                                                                                                                                            |
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Next.js 16.3.1 — 인증 없는 원격 코드 실행 2건** | 운영 경로 + critical. 패치는 **16.3.3**이라 패치 버전만 올리면 된다. 다만 **이 이슈가 만든 것이 아니고** 의존성 변경은 `CLAUDE.md`상 확인이 필요하므로 이 PR에 섞지 않는다. 별도 작업으로 즉시 올릴 것을 권한다 |

이 이슈의 코드 변경에서 나온 🔴은 없다.

### 🟡 권장 수정 (이 이슈 범위 밖)

| 항목                                                                       | 판단                                                                                                                                               |
| -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `withdrawal.controller.ts` — 탈퇴 요청이 **본문의 userId**를 그대로 믿는다 | 이 이슈가 고친 것과 **같은 종류의 결함**이다. 주석에 `TODO(#4 머지 후)`가 남아 있다. 탈퇴는 되돌리기 어려우므로 우선순위가 높다 — 별도 이슈로 낸다 |
| `agreement.controller.ts` — `GET /agreements/mine?userId=`와 PDF 내려받기  | 남의 동의서를 id만 알면 볼 수 있다. 서명(POST)은 가입 중이라 세션이 없어 라우트별로 다르게 판단해야 한다 — 별도 이슈로 낸다                        |

### ⚪ 무시 가능

| 항목                                    | 이유                                                                                                                                                     |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deepmerge-ts`·`mysql2`·`fast-uri`·`qs` | 대장(`docs/result/security-exceptions.md`)에 판정과 재검토 날짜가 이미 있다. 재검토일(2026-11-30) 전이다                                                 |
| `multer` high 3건 + low 1건             | `@nestjs/platform-express`가 끌어오지만 **업로드 라우트가 하나도 없다** — `FileInterceptor`·`@UploadedFile`이 코드 전체에 없어 그 경로가 실행되지 않는다 |
| `js-yaml`                               | 도달 경로가 `@commitlint/cli > cosmiconfig`다. 커밋 훅 전용이라 서버가 실행하지 않는다                                                                   |
| `sharp`                                 | `next > sharp`. 이미지 최적화 경로이고 이 서비스는 사용자 업로드 이미지를 다루지 않는다. Next 업그레이드와 함께 해소된다                                 |
...
| `NEXT_PUBLIC_` 오용   | 없음. 이번 변경은 환경변수를 건드리지 않는다                                                                                                  |
| 개인정보 로깅         | 없음. `console.log` 추가 없음                                                                                                                 |
| 쿠키 옵션             | 갱신 쿠키가 `httpOnly`·`secure`·`sameSite: 'lax'`·`path: '/'`로 `login.controller.ts`와 **한 글자도 다르지 않다.** 테스트가 옵션까지 단언한다 |
| 에러 응답의 내부 정보 | 401 응답은 `errorCode`와 안내 문구뿐이다. 회원이 실재하는지도 새지 않는다                                                                     |
| 입력 검증             | 컨트롤러가 여전히 zod로 `parse`한다. 회원 id만 몸체에서 가드로 옮겼다                                                                         |
