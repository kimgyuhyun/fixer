...
## AC 검증 (`@ac-verifier 33`, Green 이후)

**AC 5개 전부 충족.** 갭 없음, 추가 시나리오 없음.

가짜 테스트(단언 없는 테스트, `await` 누락, 목 반환값을 그대로 단언, 인자 없는
`rejects.toThrow()`) 점검에서도 걸린 것이 없다. AC3·AC4의 핵심 테스트가 진짜
Postgres와 진짜 `PrismaSuspensionReader`를 태운다는 점이 특히 확인됐다 —
`SuspensionReader`를 대역으로 바꿨으면 AC4는 아무것도 검증하지 않는 테스트가 된다.

지적된 것은 하나다 — **`endAt`이 정확히 `now`인 순간**을 이 이슈의 시나리오가
따로 다루지 않는다. 1밀리초 뒤가 포함된다는 것만 본다. 다만 그 판정은 #25가 만든
`activeSuspensionAtWhere`가 하고 `isSuspensionActive`의 테스트가 이미 덮고 있어
**판정에 영향이 없어 시나리오를 늘리지 않았다.**

---

## Refactor — 한 곳만 고쳤다

바뀐 파일 9개(`git diff main...HEAD`, 테스트·생성물 제외)를 중복·네이밍·단일책임·
불필요한 복잡도·컨벤션 불일치로 훑었다.

| 후보                                                   | 판단                                                                                                                               |
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| `withPenalties`가 줄마다 경고 배열 전체를 훑음         | **고쳤다.** 주석은 "한 번에 붙인다"고 말하는데 코드는 행마다 전체를 스캔했다. 회원별 `Map`으로 한 번만 가른다                      |
| `AdminError`가 `admin-job-post.service.ts`에 있음      | **범위 밖이다.** 옮기려면 이번 이슈가 건드리지 않은 파일과 그 테스트를 함께 고쳐야 한다. 관리자 화면 넷이 다 생긴 뒤가 맞는 자리다 |
| `toHttpError`가 관리자 컨트롤러 둘에 비슷하게 있음     | 2회다. **세 번째부터 뽑는다** — #34가 세 번째를 만들 때 함께 본다                                                                  |
| 목록 표·필터·다이얼로그가 `AdminJobPostList`와 닮음    | #32가 `FilterableList` 공통 컴포넌트로 만들기로 이미 정해져 있다 (`issues.md` #32)                                                 |
| `page.module.css`가 `admin/job-posts`의 것과 거의 같음 | 위와 같은 이유. 지금 공통화하면 #32가 두 번 뜯는다                                                                                 |

---

## 커밋 전 점검 (`/security-review 33`)
...
이름 검색은 Prisma `contains`로 나가므로 매개변수화된 질의다. 생 SQL을 쓰지 않는다.

**관리자 판정이 이 이슈의 코드에 없다는 점이 오히려 안전한 자리다.** 목록도 해제도
`AdminGuard`가 요청마다 DB에서 `role`을 읽는 #35의 경로를 그대로 탄다 —
`role`을 토큰에 복사하지 않았으므로 권한 회수가 즉시 반영된다.

감사 로그의 `adminId`는 **본문이 아니라 세션에서** 온다. 컨트롤러 테스트가 본문에
`adminId: 'usr_forged'`를 실어 보내 그 값이 쓰이지 않는 것을 확인한다.

### 🟡 권장 수정 — Next.js critical 2건이 새로 떴다 (이 이슈가 도입한 것이 아니다)

`pnpm audit`이 17건을 뱉는데, `security-exceptions.md`에 이미 판정된 9건
(`deepmerge-ts` 1 · `mysql2` 2 · `fast-uri` 4 · `qs` 2) 외에 **8건이 새로 떴다.**
**이 브랜치는 `package.json`도 잠금 파일도 건드리지 않았다** — 같은 버전에 권고가
새로 붙은 것이다.

| 패키지    | 심각도             | 도달 경로                  | 판정                                   |
| --------- | ------------------ | -------------------------- | -------------------------------------- |
| `next`    | critical × 2       | `apps/web` 런타임          | 🔴 **레포 차원. 사람이 판단해야 한다** |
| `multer`  | high × 3 · low × 1 | `@nestjs/platform-express` | 🟡 파일 업로드를 쓰는 라우트가 0건이다 |
| `js-yaml` | high               | 빌드·CLI                   | ⚪                                     |
| `sharp`   | high               | Next 이미지 최적화         | 🟡 `next/image`를 쓰는 화면이 없다     |

**`next`가 유일하게 성격이 다르다.** `>=16.0.0 <16.3.3`의 이미지 최적화 API
원격 코드 실행이고 현재 `16.3.1`이다. 패치는 **`16.3.3` 이상으로 올리는 것뿐**이라
위험이 낮지만, **의존성 변경은 사람이 판단한다**는 규칙에 걸리고 이 이슈의 범위도
아니라 손대지 않았다.

|                 |                                                               |
| --------------- | ------------------------------------------------------------- |
| **권고 ID**     | GHSA-2xp9-vwfh-vxw4 외 1건                                    |
| **해소 조건**   | `apps/web/package.json`의 `next`를 `16.3.3` 이상으로 올린다   |
| **재판정 시점** | **첫 배포 직전.** 배포되면 "노출 면이 없다"는 근거가 사라진다 |

### ⚪ 무시 가능 — 나머지 9건

전부 `security-exceptions.md`에 이미 판정돼 있다. 대장에 새 항목을 더하지 않았다 —
위 넷은 성격이 각각 달라 한 줄로 묶을 수 없고, `next`는 수용이 아니라 **미결**이다.

### 동시성 — 두 관리자가 같은 제재를 동시에 풀면

**조건부 `UPDATE ... WHERE releasedAt IS NULL`이 막는다.** 사전 조회가 아니라 갱신
자체가 판정이므로, 진 쪽은 갱신 건수 0을 보고 `ADMIN_SUSPENSION_ALREADY_RELEASED`로
떨어진다. 통합 테스트가 그 경로를 실제 DB에서 태우고 **감사 로그가 정확히 1건**임을
확인한다.

감사 로그가 해제와 같은 트랜잭션이라, 로그 쓰기가 실패하면 해제도 함께 되돌아간다.
없는 관리자 id로 외래키를 일부러 깨뜨려 그것을 실측했다.
