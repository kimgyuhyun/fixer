...
`openPostcodePopup`이 스텁으로 남아 있어 AC1이 FAIL 판정을 받았다. 테스트가 이 모듈을
모킹하고 있어서 초록불이었지만, 실제 브라우저에서는 팝업이 뜨지 않는다.
...
**커버리지: AC 3개 전부 커버 / 시나리오 32개 / 미커버 0개**
