...
> 상태: AC 검증 · Refactor · 보안 점검 완료 — 시나리오 33개 전부 통과
...
**AC 5개 중 4개 ✅ · 1개 ⚠️ → 보완 후 5개 ✅.** 구현한 사람과 다른 눈으로 코드와
테스트를 읽고 실제로 돌려(`apps/api` 896건 · `apps/web` 130건 · `packages/shared`
62건, 통합은 진짜 Postgres) 확인했다.

지적은 **AC2 하나**였다. `APPROVED` 전이 자체는 확실히 검증돼 있었지만, **승인이
`EXCHANGE_APPROVE`라는 조치 이름으로 감사 로그에 남는지**를 서비스를 거쳐 확인한
테스트가 없었다. 승인(`approve`)과 이체 완료(`complete`)는 `move()` 한 몸통을
공유하고 다른 것이 조치 이름 하나뿐이라, **둘이 뒤바뀌어도 나머지 31개가 전부
초록불로 남는** 상태였다. 그러면 감사 로그의 "무엇을 했나"가 틀린 채로 쌓인다.

시나리오 2개를 더해 닫았다. 구현이 이미 있었으므로 **승인의 조치 이름을 잠시
`EXCHANGE_COMPLETE`로 바꿔 빨간불을 확인하고 되돌리는** 방식으로 새 테스트가 실제로
무언가를 지키는지 증명했다 (#31이 커버리지 갭을 닫을 때와 같은 절차다).

가짜 테스트 점검(단언 없는 테스트, `await` 누락, 목 값을 그대로 단언)에서 걸린 것은
없었다. 다만 통합 테스트의 감사 로그 케이스는 서비스가 아니라 저장소를 직접 부르며
조치 이름을 테스트가 넘긴다 — **저장소가 받은 값을 정확히 기록하는지**를 보는
테스트이고, 흐름이 옳은 이름을 만드는지는 위에 더한 서비스 테스트가 맡는다.

---

## Refactor — 두 곳을 고쳤다

바뀐 파일 10개(`git diff main...HEAD`, 테스트·생성물 제외)를 중복·네이밍·단일책임·
불필요한 복잡도·컨벤션 불일치로 훑었다.

| 대상                                                      | 기준          | 무엇을 했나                                                                                                                                                 |
| --------------------------------------------------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prisma-admin-exchange.store.ts` — 같은 `select` 목록 3벌 | 중복 제거     | `RECORD_COLUMNS` 상수로 뺐다. **세 번째라 뽑았다** — 두 번이었으면 그냥 뒀다                                                                                |
| 같은 파일 — 반려의 감사 로그 조치가 문자열 리터럴         | 컨벤션 불일치 | `ADMIN_ACTIONS.EXCHANGE_REJECT`로 바꿨다. 상수 표를 만든 이유가 **오타 하나가 조용히 다른 조치로 남는 것**을 막으려는 것인데 이 한 곳만 표를 안 타고 있었다 |
| 컨트롤러의 `approve`/`complete`가 거의 같은 모양          | 중복 제거     | **안 뽑았다.** 2회이고, 라우트가 갈리는 자리라 묶으면 오히려 읽기 어렵다                                                                                    |
| `try/catch` + `toHttpError`가 네 메서드에 반복            | 컨벤션 불일치 | **안 뽑았다.** #35의 `admin-job-post.controller.ts`가 같은 모양이다 — 여기만 다르게 쓰면 그게 불일치다                                                      |

고친 뒤 전체 테스트 1088건 통과, 빌드·타입 검사 통과를 확인했다.

---

## 커밋 전 점검 (`/security-review 34`)
...
**이 이슈에서 특히 본 것은 평문 계좌번호다.** AC4가 "전체가 보인다"를 요구하므로
평문이 응답에 실리는 경로가 처음 생긴다. 넷을 확인했다.

| 확인                | 결과                                                                                                      |
| ------------------- | --------------------------------------------------------------------------------------------------------- |
| 목록에 안 섞이는가  | `listAll`이 `accountNumberLast4`만 `select`한다. 평문은 **쿼리에 아예 없다**                              |
| 누가 부를 수 있는가 | `AdminGuard`가 클래스에 걸려 있고, 관리자가 아니면 403이라는 것을 컨트롤러 테스트가 진짜 HTTP로 확인한다  |
| 흔적이 남는가       | 열람 **성공 후에만** `EXCHANGE_ACCOUNT_REVEAL` 감사 로그를 쓴다. 실패하면 로그도 없다                     |
| 로그·캐시로 새는가  | `console.*`·로거에 안 들어간다. `POST`라 브라우저·프록시가 캐시하지 않고, 화면도 React 상태로만 들고 있다 |

### 🟡 권장 수정 — 없음 (이 이슈가 만든 것 중에는)

### ⚪ 무시 가능

`pnpm audit`이 17건(critical 2 · high 11 · moderate 3 · low 1)을 보고한다. **이 이슈가
만든 것이 하나도 없다** — 의존성을 건드리지 않았고, 전부 `main`에 이미 있던 것이다.
`deepmerge-ts`·`mysql2`·`fast-uri`·`qs`는 `security-exceptions.md`에 판정이 있다.

**대장에 없는 것이 하나 있다 — `multer` < 2.3.0 (high).** `@nestjs/platform-express`가
끌어오는 운영 경로 의존성이라 성질이 `qs`와 같다. 다만 **이 이슈가 만든 것이 아니고
파일 업로드 라우트가 아직 없어** 여기서 판정하지 않고 넘긴다. 대장이 이미 "3건 이하"
규칙을 넘긴 상태(4건)이므로, **의존성 정리를 별도 이슈로 다루는 편이 맞다.**
